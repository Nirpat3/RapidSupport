...
import { registerEmbedIframeRoutes } from "./routes/embed-iframe.routes";
import { registerConversationFilesRoutes } from "./routes/conversation-files.routes";
...
const createKnowledgeCollectionSchema = z.object({
  ...
  visibility: z
    .enum(["private", "organization", "shared"])
    .default("organization"),
  defaultLocale: z.string().default("en"),
  supportedLocales: z.array(z.string()).default(["en"]),
});

const updateKnowledgeCollectionSchema =
  createKnowledgeCollectionSchema.partial();
  ...
  fileFilter: function (req, file, cb) {
    ...
    if (fileExt && validExtensions.includes(fileExt)) {
      ...
      const error = new Error(
        `Unsupported file type: ${file.mimetype}. Only PDF, TXT, DOCX, and image files are supported.`,
      );
...
async function processFileForAITraining(
  ...
  try {
    console.log(
      `Processing file ${uploadedFile.originalName} for AI training...`,
    );
    ...
    if (!supportedMimeTypes.includes(uploadedFile.mimeType)) {
      console.log(
        `Skipping AI training for ${uploadedFile.originalName} - unsupported type ${uploadedFile.mimeType}`,
      );
    ...
    console.log(
      `Successfully extracted ${documentContent.metadata?.wordCount || 0} words from ${uploadedFile.originalName}`,
    );
    ...
    console.log(
      `Created knowledge base article ${knowledgeArticle.id} for file ${uploadedFile.originalName}`,
    );
    ...
    setImmediate(async () => {
      try {
        ...
        console.log(
          `✅ Successfully indexed article ${knowledgeArticle.id} for AI search`,
        );
        ...
        console.error(
          `⚠️ Warning: Failed to index article ${knowledgeArticle.id}:`,
          indexError,
        );
      }
    });
    ...
    console.log(
      `Successfully processed file ${uploadedFile.originalName} for AI training`,
    );
  } catch (error) {
...
async function processDocumentImport(
  ...
  try {
    console.log(
      `[DocImport] Starting document import job ${importJobId} for ${uploadedFile.originalName}`,
    );
    ...
    console.log(
      `[DocImport] Extracted ${documentContent.metadata?.wordCount || 0} words from ${uploadedFile.originalName}`,
    );
...
- Minimum 2 sections, maximum 10 sections per source file`,
    ...
    if (jsonMatch) {
      try {
        ...
        console.log(
          "[DocImport] Could not parse section analysis, using full document",
        );
      }
    }
    ...
    console.log(
      `[DocImport] Identified ${sections.length} atomic document(s) to create`,
    );
    ...
    const atomicDocs: (AtomicDocument & {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (section.startText && section.endText) {
        ...
        if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
          sectionContent = fullContent.substring(
            startIdx,
            endIdx + section.endText.length,
          );
        }
      }
...
- This document will be retrieved by AI agents, so be specific and actionable`,
    ...
    for (const atomicDoc of atomicDocs) {
      ...
      const roleAccess =
        atomicDoc.accessLevel === "restricted"
          ? ["admin"]
      ...
      const safeTags = Array.isArray(atomicDoc.tags)
        ? atomicDoc.tags.filter(
            (t: any) => typeof t === "string" && t.length > 0,
          )
        : typeof atomicDoc.tags === "string"
          ? [atomicDoc.tags]
          : [];
      ...
      const version = await storage.createDocumentVersion({
        ...
        frontMatter: atomicDoc.yamlFrontmatter
          ? JSON.parse(`{"raw": ${JSON.stringify(atomicDoc.yamlFrontmatter)}}`)
          : null,
    ...
    console.log(
      `[DocImport] Successfully completed import job ${importJobId} - created ${documentsCreated} atomic documents`,
    );
  } catch (error) {
    console.error(
      `[DocImport] Error processing import job ${importJobId}:`,
      error,
    );
    await storage.updateDocumentImportJob(importJobId, {
      status: "failed",
...
export async function registerRoutes(
  app: Express,
  sessionStore?: any,
): Promise<{ server: Server; wsServer?: any }> {
  // Serve static files from public folder (for embed widget)
  app.use(
    "/embed",
    express.static(path.join(process.cwd(), "public", "embed")),
  );
  ...
  const conversationCreateLimiter = rateLimit({
    ...
    message: {
      error:
        "You have reached the maximum number of new conversations. Please try again later.",
    },
    standardHeaders: true,
    legacyHeaders: false,
  });
    ...
    if (origin && host) {
      ...
      if (originHost.toLowerCase() !== hostWithoutPort.toLowerCase()) {
        ...
        if (
          ...
          originHost.includes(".kirk.replit.dev")
        ) {
          console.log("[CSRF] Allowing Replit domain:", originHost);
        } else {
          console.log(
            "[CSRF] Origin mismatch - Origin:",
            originHost,
            "Host:",
            hostWithoutPort,
          );
  ...
  app.get("/api/geo/detect", async (req, res) => {
    try {
      ...
      if (xVercelCountry) {
        return res.json({
          countryCode: xVercelCountry.toUpperCase(),
          source: "vercel",
        });
      }
  ...
  app.get("/api/customer-portal/stats", async (req, res) => {
    try {
      ...
      const conversations =
        await storage.getConversationsByCustomer(customerId);
  ...
  app.get("/api/customer-portal/unread-counts", async (req, res) => {
    try {
      ...
      const unreadCounts =
        await storage.getUnreadMessageCountsPerConversation(customerId);
  ...
  app.get("/api/customer-portal/conversations", async (req, res) => {
    try {
      ...
      if (
        ...
        customer.customerOrgRole === "admin"
      ) {
        conversations = await storage.getConversationsByCustomerOrganization(
          customer.customerOrganizationId,
        );
      } else {
      ...
      const unreadMap = new Map(
        unreadCounts.map((u) => [u.conversationId, u.unreadCount]),
      );
      ...
      const agentIds = [
        ...new Set(
          conversations
            ...
            .map((c) => c.assignedAgentId as string),
        ),
      ];
      ...
      const allConversations = conversations
        ...
        .map((conv) => ({
          ...
          assignedAgentName: conv.assignedAgentId
            ? agentMap.get(conv.assignedAgentId)?.name || "Support Agent"
            : null,
  ...
  app.post(
    "/api/customer-portal/conversations/create",
    ...
    async (req, res) => {
      try {
        ...
        if (createData.categoryId && createData.categoryId !== "general") {
          supportCategory = await storage.getSupportCategory(
  ...
  app.get(
    "/api/customer-portal/conversation/:conversationId",
    async (req, res) => {
      try {
        const customerId = (req.session as any).customerId;
        const userType = (req.session as any).userType;
        ...
        storage
          ...
          .catch((err) =>
            console.error("Failed to update customerLastViewedAt:", err),
          );
  ...
  app.post(
    "/api/customer-portal/conversation/:conversationId/read",
    async (req, res) => {
      try {
        const customerId = (req.session as any).customerId;
        const userType = (req.session as any).userType;
        ...
        await storage.markAllConversationMessagesAsRead(
          conversationId,
          customerId,
        );
  ...
  app.post(
    "/api/customer-portal/conversation/:conversationId/reopen",
    async (req, res) => {
      try {
        const customerId = (req.session as any).customerId;
        const userType = (req.session as any).userType;
        ...
        if (
          ...
          return res
            .status(400)
            .json({ error: "Conversation is already open" });
        }
  ...
  app.get(
    "/api/customer-portal/conversation/:conversationId/history",
    async (req, res) => {
      try {
        const customerId = (req.session as any).customerId;
        const userType = (req.session as any).userType;
        ...
        const activityLogs =
          await storage.getActivityLogsByConversation(conversationId);
  ...
  app.post(
    "/api/customer-portal/conversation/:conversationId/messages",
    messageLimiter,
    async (req, res) => {
      try {
        const customerId = (req.session as any).customerId;
        ...
        if (
          ...
          console.log(
            `[portal-chat] Reopening closed conversation: ${conversationId}`,
          );
          await storage.updateConversationStatus(conversationId, "open");
        }
        ...
        if (customerLanguage !== "en") {
          console.log(
            `[portal-chat] Customer language is ${customerLanguage}, translating to English`,
          );
          ...
          if (detection.language !== "en" && detection.confidence > 70) {
            console.log(
              `[portal-chat] Detected non-English: ${detection.language}`,
            );
        ...
        if (wsServer && wsServer.broadcastNewMessageToStaff) {
          const fullConversation =
            await storage.getConversationWithCustomer(conversationId);
          if (fullConversation && fullConversation.customer) {
            wsServer.broadcastNewMessageToStaff(
              fullConversation,
              fullConversation.customer,
              {
                id: message.id,
        ...
        if (wsServer && wsServer.broadcastNewMessage) {
          ...
          console.log(
            `[portal-chat] Broadcast new_message event for customer message: ${message.id} to ${staffUserIds.length} staff`,
          );
        ...
        const isAiEnabledForPortal =
          engagementSettings?.aiGlobalEnabled !== false &&
          engagementSettings?.aiCustomerPortalEnabled !== false;

        if (
          conversation &&
          conversation.aiAssistanceEnabled !== false &&
          isAiEnabledForPortal
        ) {
          (async () => {
            try {
              console.log(
                `[portal-chat] Generating AI response for conversation: ${conversationId}`,
              );
              const aiResponse = await AIService.generateSmartAgentResponse(
                content,
                conversationId,
              ...
              if (aiResponse && aiResponse.response) {
                ...
                console.log(
                  `[portal-chat] AI response created: ${aiMessage.id}`,
                );
              ...
              console.error(
                "[portal-chat] AI response generation failed:",
                aiError,
              );
            }
          })();
          ...
          console.log(
            `[portal-chat] AI response skipped - AI is disabled for conversation: ${conversationId} (learning mode)`,
          );
  ...
  app.get("/api/customer-portal/organization", async (req, res) => {
    try {
      ...
      const org = await storage.getCustomerOrganization(
        customer.customerOrganizationId,
      );
      if (!org) {
        return res.json({ hasOrganization: false });
      ...
      if (customer.customerOrgRole === "admin") {
        const orgCustomers = await storage.getCustomersByOrganization(
          customer.customerOrganizationId,
        );
  ...
  app.put(
    "/api/customer-portal/organization/members/:memberId/role",
    async (req, res) => {
      try {
        const customerId = (req.session as any).customerId;
        const userType = (req.session as any).userType;
        ...
        if (
          !member ||
          member.customerOrganizationId !== customer.customerOrganizationId
        ) {
          return res
            .status(404)
        ...
        if (roleData.role === "member" && member.customerOrgRole === "admin") {
          const orgMembers = await storage.getCustomersByOrganization(
            customer.customerOrganizationId,
          );
          const adminCount = orgMembers.filter(
            (m) => m.customerOrgRole === "admin",
          ).length;
          if (adminCount <= 1) {
            return res
              .status(400)
        ...
        await storage.updateCustomerOrganizationMembership(
          memberId,
          customer.customerOrganizationId,
          roleData.role,
        );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const organizationId = currentUser.isPlatformAdmin
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const invites =
          await storage.getStaffInvitesByOrganization(organizationId);
        res.json(invites);
      } catch (error) {
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        const invites = await storage.getStaffInvitesByOrganization(
          currentUser.organizationId,
        );
  ...
  app.post("/api/users/me/select-workspace", requireAuth, async (req, res) => {
    try {
      ...
      const membership = await storage.getWorkspaceMemberByUserAndWorkspace(
        user.id,
        workspaceId,
      );
  ...
  app.get("/api/users/me", requireAuth, async (req, res) => {
    try {
      ...
      const { password, twoFactorSecret, twoFactorBackupCodes, ...safeUser } =
        user as any;
      res.json(safeUser);
    } catch (error) {
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const newStatus =
          existingUser.status === "disabled" ? "offline" : "disabled";
  ...
  app.get(
    "/api/permissions/users-with-permissions",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch users with permissions" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const { userId, feature, permission } = z
          .object({
            userId: z.string(),
            feature: z.string(),
  ...
  app.delete(
    "/api/permissions/:userId/:feature",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.get("/api/conversations", requireAuth, async (req, res) => {
    try {
      ...
      if (user.role === "admin" || user.role === "agent") {
        ...
        console.log(
          `${user.role} ${user.name} requesting conversations, found ${allConversations.length} conversations`,
        );
      ...
      console.log(
        `Starting enrichment for ${conversations.length} conversations`,
      );
      const enrichedConversations = await Promise.all(
        conversations.map(async (conv) => {
          try {
            ...
            if (lastMessage) {
              ...
              lastMessageData = {
                ...
                timestamp: lastMessage.timestamp,
                sender:
                  lastMessage.senderType === "customer" ? "customer" : "agent",
              };
            }
      ...
      console.log(
        `Enrichment completed, ${enrichedConversations.length} conversations enriched`,
      );

      console.log(
        `Sending ${enrichedConversations.length} conversations to client`,
      );
  ...
  app.get("/api/conversations/tags", requireAuth, async (req, res) => {
    try {
      ...
      const organizationId =
        user.organizationId || (req.session as any).selectedOrganizationId;
      ...
      try {
        ...
        if (previousAgentId && previousAgentId !== agentId) {
          ...
          const previousWorkload =
            await storage.getAgentWorkload(previousAgentId);
          if (previousWorkload) {
            await storage.updateAgentWorkload(
              previousAgentId,
              Math.max(0, previousWorkload.activeConversations - 1),
            );
          }
        }
        ...
        if (newWorkload) {
          await storage.updateAgentWorkload(
            agentId,
            newWorkload.activeConversations + 1,
          );
        }
    ...
    async (req, res) => {
      try {
        ...
        if (conversation.assignedAgentId) {
          return res
            .status(400)
            .json({ error: "Conversation is already assigned" });
        }
        ...
        if (
          agentWorkload &&
          agentWorkload.activeConversations >= agentWorkload.maxCapacity
        ) {
          return res
            .status(400)
            .json({ error: "Agent has reached maximum capacity" });
        }
        ...
        if (agentWorkload) {
          await storage.updateAgentWorkload(
            user.id,
            agentWorkload.activeConversations + 1,
          );
        }
  ...
  app.put(
    "/api/notifications/:conversationId/read",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as any;
  ...
  app.get("/api/unread-counts", requireAuth, async (req, res) => {
    try {
      const user = req.user as any;
      const unreadCounts = await storage.getUnreadMessageCountsPerConversation(
        user.id,
      );
      res.json(unreadCounts);
    } catch (error) {
  ...
  app.post("/api/conversations/:id/merge", requireAuth, async (req, res) => {
    try {
      ...
      if (sourceId === targetConversationId) {
        return res
          .status(400)
          .json({ error: "Cannot merge a conversation into itself" });
      }
      ...
      if (source.organizationId !== target.organizationId) {
        return res
          .status(400)
          .json({
            error: "Conversations must belong to the same organization",
          });
      }

      const result = await storage.mergeConversations(
        sourceId,
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const success = await storage.markAllConversationMessagesAsRead(
          conversationId,
          user.id,
        );
  ...
  app.get("/api/conversations/:id/messages", requireAuth, async (req, res) => {
    try {
      ...
      const readStatus = await storage.getMessagesReadStatus(
        messageIds,
        user.id,
      );
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `[PATCH /api/conversations/:id/status] Updating conversation ${conversationId} to status: ${status}`,
        );
        ...
        if (!conversation) {
          console.log(
            `[PATCH /api/conversations/:id/status] Conversation ${conversationId} not found`,
          );
          return res.status(404).json({ error: "Conversation not found" });
        }
        ...
        const previousStatus = conversation.status;
        console.log(
          `[PATCH /api/conversations/:id/status] Current status: ${previousStatus}, updating to: ${status}`,
        );
        ...
        console.log(
          `[PATCH /api/conversations/:id/status] Status after update: ${updatedConversation?.status}`,
        );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `[POST /api/conversations/bulk/close] Closing ${conversationIds.length} conversations`,
        );
        ...
        for (const conversationId of conversationIds) {
          try {
            ...
            if (!conversation) {
              console.log(
                `[POST /api/conversations/bulk/close] Conversation ${conversationId} not found`,
              );
              continue;
            }
            ...
            console.error(
              `[POST /api/conversations/bulk/close] Error closing conversation ${conversationId}:`,
              err,
            );
            results.push({
              conversationId,
        ...
        res.json({
          message: `${conversationIds.length} conversation(s) closed successfully`,
          results,
        });
      } catch (error) {
        if (error instanceof z.ZodError) {
  ...
  app.patch(
    "/api/conversations/:id/ai-assistance",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
      ...
      try {
        ...
        if (user.role === "agent" && conversation.assignedAgentId !== user.id) {
          return res
            ...
            .json({
              error:
                "You can only set follow-ups for conversations assigned to you",
            });
        }
        ...
        let systemMessageContent = "";
        if (followupDate) {
          const followupDateFormatted = new Date(
            followupDate,
  ...
  app.post("/api/conversations/:id/rating", async (req, res) => {
    try {
      ...
      const { rating, feedback, customerName, customerEmail } =
      ...
      const existingRating =
        await storage.getConversationRating(conversationId);
      if (existingRating) {
      ...
      const sentimentAnalysis =
        await AIService.analyzeConversationSentiment(messages);
  ...
  app.get("/api/conversations/:id/rating", async (req, res) => {
    try {
      ...
      if (!rating) {
        return res
          .status(404)
          .json({ error: "No rating found for this conversation" });
      }
      ...
      try {
        ...
        if (user.role === "agent" && user.id !== agentId) {
          return res
            .status(403)
            .json({
              error: "You can only view your own performance statistics",
            });
        }
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch performance statistics" });
      }
    },
  );
  ...
  app.post(
    "/api/agents/:agentId/performance/calculate",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        const stats = await storage.calculateAndStoreAgentStats(
          agentId,
          periodStart,
          periodEnd,
        );
        ...
        res
          .status(500)
          .json({ error: "Failed to calculate performance statistics" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const stats = await storage.getAllAgentsPerformanceStats(
          periodStart,
          periodEnd,
        );
  ...
  app.patch(
    "/api/activity/notifications/:id/read",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.patch(
    "/api/activity/notifications/read-all",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as any;
      ...
      try {
        const notificationData = z
          .object({
            ...
            linkType: z
              .enum(["conversation", "post", "knowledge_base", "custom"])
              .optional(),
        ...
        const notification =
          await storage.createActivityNotification(notificationData);
  ...
  app.post("/api/messages", requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `[POST /api/messages] Staff ${user.name} (${user.role}) sending message to conversation ${conversationId}`,
      );
      ...
      if (conversation.status === "closed") {
        console.log(
          `[messages] Reopening closed conversation: ${conversationId}`,
        );
        await storage.updateConversationStatus(conversationId, "open");
      }
      ...
      if (customerLanguage && customerLanguage !== "en") {
        console.log(
          `[POST /api/messages] Translating agent message from English to ${customerLanguage}`,
        );
        const translation = await AIService.translateText(
          content,
          customerLanguage,
        ...
        translatedContent = translation.translatedText;
        console.log(
          `[POST /api/messages] Translation complete: "${content.substring(0, 50)}..." → "${translatedContent?.substring(0, 50)}..."`,
        );
      }

      console.log(
        `[POST /api/messages] Creating message with senderId=${user.id}, senderType=${senderType}, scope=public`,
      );
      const message = await storage.createMessage({
        conversationId,
        senderId: user.id,
      ...
      console.log(
        `[POST /api/messages] Message created successfully: ${message.id}`,
      );
      ...
      if (wsServer) {
        ...
        const targetUserIds = conversation.customerId
  ...
  app.get(
    "/api/conversations/:id/internal-messages",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        if (user.role === "agent" && conversation.assignedAgentId !== user.id) {
          return res
            ...
            .json({
              error:
                "You can only view internal messages for conversations assigned to you",
            });
        }

        const messages = await storage.getMessagesByConversationAndScope(
          conversationId,
          "internal",
        );
      ...
      try {
        ...
        if (user.role === "agent" && conversation.assignedAgentId !== user.id) {
          return res
            ...
            .json({
              error:
                "You can only send internal messages for conversations assigned to you",
            });
        }
  ...
  app.post("/api/messages/:messageId/rate", async (req, res) => {
    try {
      ...
      if (rating !== "like" && rating !== "dislike") {
        return res
          .status(400)
          .json({ error: 'Rating must be either "like" or "dislike"' });
      }
      ...
      // Rate the message
      const messageRating = await storage.rateMessage(
        messageId,
        userId,
        customerId,
        rating,
  ...
  app.get("/api/messages/:messageId/rating", async (req, res) => {
    try {
      ...
      const user = req.user as any;
      if (user) {
        const userRating = await storage.getMessageRating(
          messageId,
          user.id,
          null,
        );
  ...
  app.get("/api/dashboard/stats", requireAuth, async (req, res) => {
    try {
      ...
      const onlineCustomers = customers.customers.filter(
        (c: any) => c.status === "online",
      ).length;
      const totalAgents = users.filter((u) => u.role === "agent").length;
      const onlineAgents = users.filter(
        (u) => u.role === "agent" && u.status === "online",
      ).length;
  ...
  app.get(
    "/api/conversations/by-status/:status",
    requireAuth,
    async (req, res) => {
      try {
        const conversations = await storage.getAllConversations();
        const filtered = conversations.filter(
          (c) => c.status === req.params.status,
        );
        res.json(filtered);
      } catch (error) {
        res
          .status(500)
          .json({ error: "Failed to fetch conversations by status" });
      }
    },
  );

  app.get(
    "/api/conversations/by-agent/:agentId",
    requireAuth,
    async (req, res) => {
      try {
        const conversations = await storage.getAllConversations();
        const filtered = conversations.filter(
          (c) => c.assignedAgentId === req.params.agentId,
        );
        res.json(filtered);
      } catch (error) {
        res
          .status(500)
          .json({ error: "Failed to fetch conversations by agent" });
      }
    },
  );
  ...
  app.get(
    "/api/customers/:customerId/kb-suggestions",
    requireAuth,
    async (req, res) => {
      try {
        ...
        let contextQuery = "";
        if (conversationId) {
          const msgs = await storage.getMessagesByConversation(
        ...
        const articles = await storage.searchKnowledgeBase(
          contextQuery || "frequently asked questions",
          6,
        );
        const filtered = articles.filter(
          (a) => !a.organizationId || a.organizationId === organizationId,
        );
        res.json(
          filtered
            .slice(0, 5)
    ...
    async (req, res) => {
      try {
        ...
        if (wsServer && wsServer.broadcastNewConversation) {
          wsServer.broadcastNewConversation(
            conversation,
            customer,
            "New conversation started by agent",
          );
        }
        ...
        if (
          ...
          console.log(
            `[conversations] Agent ${user.name} started conversation with customer ${customer.name}, initial message: ${message.id}`,
          );
        }
  ...
  app.get("/manifest.json", async (req, res) => {
    try {
      ...
      const defaultManifest = {
        ...
        description:
          "Professional customer support platform with real-time chat, conversation management, and admin dashboard.",
        start_url: "/",
        display: "standalone",
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const applications =
          await storage.getAllOrganizationApplications(status);
        res.json(applications);
      } catch (error) {
        console.error("Error fetching organization applications:", error);
        res
          .status(500)
          .json({ error: "Failed to fetch organization applications" });
      }
    },
  );
  ...
  app.get(
    "/api/admin/organization-applications/:id",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        const application = await storage.getOrganizationApplication(
          req.params.id,
        );
        if (!application) {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch organization application" });
      }
    },
  );
  ...
  app.post(
    "/api/admin/organization-applications/:id/approve",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        res.json({
          ...
          setupUrl,
          message:
            "Application approved. Share the setup link with the contact person.",
        });
      } catch (error) {
  ...
  app.post(
    "/api/admin/organization-applications/:id/reject",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const existingOrg = await storage.getOrganizationBySlug(
          data.organizationSlug,
        );
        if (existingOrg) {
          return res
            .status(400)
            .json({ error: "Organization slug is already in use" });
        }
        ...
        res.json({
          ...
          setupUrl,
          message:
            "Setup link generated. Share with the contact person to complete organization setup.",
        });
      } catch (error) {
        if (error instanceof z.ZodError) {
          return res
  ...
  app.get(
    "/api/admin/organization-setup-tokens",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.delete(
    "/api/admin/organization-setup-tokens/:id",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        await storage.updateOrganizationSetupToken(req.params.id, {
          status: "revoked",
        });
  ...
  app.get("/api/public/organization-setup/:token", async (req, res) => {
    try {
      const setupToken = await storage.getOrganizationSetupTokenByToken(
        req.params.token,
      );
      ...
      if (setupToken.status === "revoked") {
        return res
          .status(400)
          .json({ error: "This setup link has been revoked" });
      }

      if (new Date() > new Date(setupToken.expiresAt)) {
        await storage.updateOrganizationSetupToken(setupToken.id, {
          status: "expired",
        });
  ...
  app.post(
    "/api/public/organization-setup/:token/complete",
    async (req, res) => {
      try {
        const setupToken = await storage.getOrganizationSetupTokenByToken(
          req.params.token,
        ...
        if (!setupToken) {
          return res
            .status(404)
            .json({ error: "Invalid or expired setup link" });
        }

        if (setupToken.status !== "pending") {
          return res
            .status(400)
            .json({ error: "Setup link is no longer valid" });
        }
        ...
        if (setupToken.applicationId) {
          await storage.updateOrganizationApplication(
            setupToken.applicationId,
            {
        ...
        res
          .status(500)
          .json({ error: "Failed to complete organization setup" });
      }
    },
  );
  ...
  app.get(
    "/api/customer-chat-placeholder-messages/:conversationId",
    async (req, res) => {
      try {
        const { conversationId } = req.params;
        console.log(
          `[customer-chat/messages] Fetching messages for conversation: ${conversationId}`,
        );
        const messages = await storage.getCustomerChatMessages(conversationId);
        console.log(
          `[customer-chat/messages] Returned ${messages.length} messages:`,
          messages.map((m) => ({
            id: m.id,
        ...
        console.error(
          "[customer-chat/messages] Error fetching messages:",
          error,
        );
        res.status(500).json({ error: "Failed to fetch messages" });
      }
  ...
  app.post(
    "/api/customer-chat/send-message",
    messageLimiter,
    async (req, res) => {
      try {
        ...
        if (existingConversation && existingConversation.status === "closed") {
          console.log(
            `[send-message] Reopening closed conversation: ${messageData.conversationId}`,
          );
        ...
        let customerLanguage =
          (existingConversation as any)?.customerLanguage || "en";
        let translatedContent: string | null = null;
        let originalLanguage: string | null = null;
        ...
        if (customerLanguage !== "en") {
          console.log(
            `[send-message] Customer language is ${customerLanguage}, translating to English for agents`,
          );
          const translation = await AIService.translateText(
            messageData.content,
            "en",
          ...
          console.log(
            `[send-message] Translation complete: "${messageData.content.substring(0, 50)}..." → "${translatedContent?.substring(0, 50)}..."`,
          );
        } else {
          // Detect language if we haven't set it yet and message is long enough
          if (messageData.content.length > 10) {
            const detection = await AIService.detectLanguage(
              messageData.content,
            );
            if (detection.language !== "en" && detection.confidence > 70) {
              console.log(
                `[send-message] Detected non-English language: ${detection.language} (confidence: ${detection.confidence})`,
              );
              customerLanguage = detection.language;
              originalLanguage = detection.language;
        ...
        const message =
          await storage.createCustomerMessage(enhancedMessageData);

        // Get conversation and customer details for notifications
        const conversation = await storage.getConversationWithCustomer(
          messageData.conversationId,
        );
        if (conversation && conversation.customer) {
          // Create notifications for all staff members
          await storage.createNotificationsForAllStaff(
            messageData.conversationId,
          );
  ...
  const customerChatUpload = multer({
    ...
    fileFilter: function (req, file, cb) {
      ...
      if (allowedMimes.includes(file.mimetype)) {
        ...
        cb(
          new Error(
            "Invalid file type. Only images, PDF, TXT, and DOCX are allowed",
          ),
        );
      }
    },
  ...
  app.post(
    "/api/customer-chat/upload-files",
    customerChatUpload.array("files", 5),
    async (req, res) => {
      try {
  ...
  app.post("/api/customer-chat/set-language", async (req, res) => {
    try {
      const { conversationId, language } = z
        .object({
      ...
      console.log(
        `[set-language] Setting customer language to ${language} for conversation ${conversationId}`,
      );
  ...
  app.get("/api/customer-chat/conversation/:id/status", async (req, res) => {
    try {
      ...
      console.log(
        `[GET /api/customer-chat/conversation/:id/status] Fetching status for conversation ${conversationId}`,
      );

      const conversation = await storage.getConversation(conversationId);
      if (!conversation) {
        console.log(
          `[GET /api/customer-chat/conversation/:id/status] Conversation ${conversationId} not found`,
        );
        return res.status(404).json({ error: "Conversation not found" });
      }

      console.log(
        `[GET /api/customer-chat/conversation/:id/status] Returning status: ${conversation.status} for conversation ${conversationId}`,
      );
      ...
      console.error(
        "[GET /api/customer-chat/conversation/:id/status] Error fetching conversation status:",
        error,
      );
      if (error instanceof z.ZodError) {
        return res.status(400).json({ error: "Invalid conversation ID" });
    ...
    async (req, res) => {
      try {
        ...
        if (conversationId && typeof conversationId === "string") {
          ...
          return res
            .status(400)
            .json({ error: "Agent ID or Conversation ID required" });
        }
  ...
  app.post("/api/ai/smart-response-stream", async (req, res) => {
    ...
    try {
      ...
      const { conversationId, customerMessage, customerId, agentId } =
        streamResponseSchema.parse(req.body);
      ...
      if (conversation.aiAssistanceEnabled === false) {
        console.log(
          `[${requestId}] AI disabled for this specific conversation (per-conversation toggle) - learning mode`,
        );
        ...
        return res
          .status(200)
          .json({ response: null, aiDisabled: true, learningMode: true });
      }
      ...
      const aiEnabledForContext = isAnonymous
        ? engagementSettings?.aiGlobalEnabled !== false &&
          engagementSettings?.aiAnonymousChatEnabled !== false
        : engagementSettings?.aiGlobalEnabled !== false &&
          engagementSettings?.aiCustomerPortalEnabled !== false;

      if (!aiEnabledForContext) {
        console.log(
          `[${requestId}] AI disabled for ${isAnonymous ? "anonymous chat" : "customer portal"} (global settings) - learning mode`,
        );
      ...
      try {
        ...
        console.log(
          `[${requestId}] Stream complete. Response length: ${fullResponse.length}`,
        );
        ...
        console.log(
          `[${requestId}] AI message persisted. Message ID: ${message.id}`,
        );
  ...
  app.post("/api/ai/smart-response", async (req, res) => {
    ...
    try {
      ...
      const { conversationId, customerMessage, customerId, agentId, language } =
        smartResponseSchema.parse(req.body);
      console.log(
        `[${requestId}] Validated - ConvID: ${conversationId}, Message: "${customerMessage.substring(0, 50)}..."`,
      );
      ...
      console.log(
        `[${requestId}] AI response generated. Length: ${aiResponse.response?.length || 0} chars`,
      );
      ...
      if (aiResponse.response) {
        ...
        const message = await storage.createMessage(messageData);
        console.log(
          `[${requestId}] AI message created. Message ID: ${message.id}`,
        );
        ...
        if (conversation.customer) {
          ...
          if (
            aiResponse.requiresHumanTakeover &&
            wsServer &&
            wsServer.broadcastToStaff
          ) {
            wsServer.broadcastToStaff({
      ...
      console.log(
        `[${requestId}] ===== AI SMART RESPONSE REQUEST END (SUCCESS) =====`,
      );
      res.json({
        success: true,
        data: aiResponse,
      ...
      console.log(
        `[${requestId}] ===== AI SMART RESPONSE REQUEST END (ERROR) =====`,
      );
      console.error(`[${requestId}] Error:`, error);
      if (error instanceof z.ZodError) {
        return res.status(400).json({
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch active AI conversations" });
      }
    },
  );
  ...
  app.post(
    "/api/ai/auto-handover/:conversationId",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        const { conversationId } = req.params;
        const { reason = "Automatic handover due to low AI confidence" } =
          req.body;
        ...
        if (!bestAgent) {
          return res
            .status(503)
            .json({ error: "No agents available for handover" });
        }

        // Perform handover to best agent
        const success = await AIService.handoverToHuman(
          conversationId,
          bestAgent.id,
          reason,
        );
        ...
        if (success) {
          ...
          res
            .status(400)
            .json({
              error: "No active AI session found for this conversation",
            });
        }
      } catch (error) {
  ...
  app.post(
    "/api/ai/handover/:conversationId",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
    ...
    try {
      ...
      if (!conversationId || !messageId || typeof isHelpful !== "boolean") {
        return res
          ...
          .json({
            error:
              "Conversation ID, message ID, and feedback rating are required",
          });
      }
      ...
      try {
        ...
        if (existing.length > 0) {
          return res
            ...
            .json({
              error:
                "Categories already exist. Delete existing ones first or update them.",
            });
        }
    ...
    async (req, res) => {
      try {
        ...
        if (!entryId || typeof wasHelpful !== "boolean") {
          return res
            .status(400)
            .json({ error: "Entry ID and feedback rating are required" });
        }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const { entryId, improvedResponse, reasoning, knowledgeToAdd } =
          req.body;
        const user = req.user as any;
        ...
        if (!entryId || !improvedResponse || !reasoning) {
          return res
            .status(400)
            .json({
              error: "Entry ID, improved response, and reasoning are required",
            });
        }
    ...
    async (req, res) => {
      try {
        ...
        if (!question || !agentId) {
          return res
            .status(400)
            .json({ error: "Question and agent ID are required" });
        }
        ...
        res.json({
          ...
          sources: sources.map((kb, index) => ({
            ...
            relevance: kb.effectiveness
              ? kb.effectiveness / 100
              : 0.85 - index * 0.05,
          })),
        });
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        if (!question || !correctedResponse || !reasoning || !knowledgeBaseId) {
          return res.status(400).json({
            error:
              "Question, corrected response, reasoning, and knowledge base ID are required",
          });
        }
        ...
        const latestVersion =
          await storage.getLatestVersionNumber(knowledgeBaseId);
        ...
        console.error(
          "Failed to submit correction and update knowledge base:",
          error,
        );
        res.status(500).json({ error: "Failed to submit correction" });
      }
  ...
  app.get(
    "/api/knowledge-base/:id/versions",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
    ...
    async (req, res) => {
      try {
        ...
        if (intent && intent !== "all")
          filters.intentCategory = intent as string;
        ...
        const millisecondsAgo =
          timeRangeMap[timeRange as string] || timeRangeMap["7d"];
        const startDate = new Date(now.getTime() - millisecondsAgo);

        const learningEntries = await storage.getAiLearningEntriesFiltered(
          filters,
          startDate,
        );
        ...
        const enrichedEntries = await Promise.all(
          learningEntries.map(async (entry) => {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        // Group by agent
        const agentGroups = learningEntries.reduce(
          (acc, entry) => {
        ...
        const stats = await Promise.all(
          Object.entries(agentGroups).map(async ([agentId, entries]) => {
            const agent = await storage.getAiAgent(agentId);
            const totalResponses = entries.length;
            const avgQuality = Math.round(
              entries.reduce((sum, e) => sum + (e.qualityScore || 0), 0) /
                totalResponses,
            );
            const avgTone = Math.round(
              entries.reduce((sum, e) => sum + (e.toneScore || 0), 0) /
                totalResponses,
            );
            const avgRelevance = Math.round(
              entries.reduce((sum, e) => sum + (e.relevanceScore || 0), 0) /
                totalResponses,
            );
            const avgCompleteness = Math.round(
              entries.reduce((sum, e) => sum + (e.completenessScore || 0), 0) /
                totalResponses,
            );
            const avgConfidence = Math.round(
              entries.reduce((sum, e) => sum + (e.confidence || 0), 0) /
                totalResponses,
            );
            const humanTakeovers = entries.filter(
              (e) => e.humanTookOver,
            ).length;
            const humanTakeoverRate = Math.round(
              (humanTakeovers / totalResponses) * 100,
            );
            const satisfactionEntries = entries.filter(
              (e) => e.customerSatisfaction !== null,
            );
            const avgCustomerSatisfaction =
              satisfactionEntries.length > 0
                ? satisfactionEntries.reduce(
                    (sum, e) => sum + (e.customerSatisfaction || 0),
                    0,
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const matchingItem = queueItems.find(
          (item) => item.sourceId === id && item.sourceType === "correction",
        );
        if (matchingItem) {
          await storage.updateTrainingQueueItem(matchingItem.id, {
            status: "applied",
            appliedAt: new Date(),
          });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        if (status) {
          await storage.updateAiCorrection(id, {
            appliedToKnowledge: status === "applied",
          });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        if (resolvedByArticleId)
          updates.resolvedByArticleId = resolvedByArticleId;
   ...
   */
  app.post(
    "/api/integrations/start-conversation",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch customers", success: false });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch customer", success: false });
      }
    },
  );

  app.post(
    "/api/external/customers/sync",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to sync customer", success: false });
      }
    },
  );

  app.put(
    "/api/external/customers/:id/sync-status",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({
            error: "Failed to update customer sync status",
            success: false,
          });
      }
    },
  ...
  app.post(
    "/api/conversations/:id/generate-ticket",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        if (user.role === "agent" && conversation.assignedAgentId !== user.id) {
          return res
            ...
            .json({
              error:
                "You can only generate tickets for conversations assigned to you",
            });
        }
        ...
        if (!isAiAvailable) {
          return res.status(503).json({
            error:
              "AI service is currently unavailable. Please try again later or create the ticket manually.",
          });
        }

        // Generate AI ticket suggestions
        const aiTicketData =
          await AIService.generateTicketFromConversation(conversationId);

        // Validate the AI-generated data against schema
        const validatedData = aiTicketGenerationSchema.safeParse(aiTicketData);
        if (!validatedData.success) {
          console.error(
            "AI generated invalid ticket data:",
            validatedData.error,
          );
          return res.status(500).json({
            error:
              "AI generated invalid ticket data. Please try again or create the ticket manually.",
          });
        }
        ...
        res
          .status(500)
          .json({ error: "Failed to generate AI ticket suggestions" });
      }
    },
  );
      ...
      try {
        ...
        if (ticketData.conversationId) {
          ...
          if (!conversation) {
            return res
              .status(404)
              .json({ error: "Associated conversation not found" });
          }
          ...
          if (
            ...
            return res
              ...
              .json({
                error:
                  "You can only create tickets for conversations assigned to you",
              });
          }
        }
    ...
    async (req, res) => {
      try {
        ...
        if (!isAiAvailable) {
          return res.status(503).json({
            error:
              "AI service is currently unavailable. Please check your content manually.",
          });
        }
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch tickets", success: false });
      }
    },
  );

  app.get(
    "/api/external/tickets/:id",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
        ...
        if (!ticket) {
          return res
            .status(404)
            .json({ error: "Ticket not found", success: false });
        }
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch ticket", success: false });
      }
    },
  );

  app.post(
    "/api/external/tickets/sync",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to sync ticket", success: false });
      }
    },
  );

  app.put(
    "/api/external/tickets/:id/status",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to update ticket status", success: false });
      }
    },
  );

  app.put(
    "/api/external/tickets/:id/assign",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to assign ticket", success: false });
      }
    },
  );

  app.put(
    "/api/external/tickets/:id/sync-status",
    requireApiKey,
    externalApiLimiter,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({
            error: "Failed to update ticket sync status",
            success: false,
          });
      }
    },
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to process webhook", success: false });
      }
    },
  );

  // Helper function to synchronize agent assignments
  async function syncAgentKnowledgeAssignments(
    articleId: string,
      ...
      for (const article of activeArticles) {
        if (article.title) {
          ...
          titleWords.forEach((w) =>
            keywordsSet.add(w.replace(/[^a-zA-Z0-9]/g, "")),
          );
        }
      ...
      try {
        ...
        if (indexingStatus && typeof indexingStatus === "string") {
          articles = articles.filter(
            (article) => article.indexingStatus === indexingStatus,
          );
        }
  ...
  app.post(
    "/api/public/organizations/signup",
    authLimiter,
    async (req, res) => {
      try {
        const signupSchema = z.object({
        ...
        if (existingOrg) {
          return res
            .status(400)
            .json({ error: "This organization URL is already taken" });
        }
  ...
  app.post("/api/public/organizations/check-availability", async (req, res) => {
    try {
      ...
      const result: {
        ...
        duplicateOrg?: { name: string };
      } = {};

      if (name) {
        const duplicate = await storage.checkOrganizationDuplicate(
          name,
          website,
        );
  ...
  app.post("/api/public/organizations/apply", authLimiter, async (req, res) => {
    try {
      ...
      res.status(201).json({
        ...
        message:
          "Application submitted successfully. We will review your application and get back to you within 2 business days.",
  ...
  app.get("/api/public/knowledge-base", async (req, res) => {
    try {
      ...
      if (category && typeof category === "string") {
        publicArticles = publicArticles.filter(
          (article) =>
    ...
    try {
      ...
      if (customerId && typeof customerId === "string") {
        try {
          const customerConversations =
            await storage.getConversationsByCustomerId(customerId);
          ...
          for (const conv of customerConversations.slice(0, 10)) {
            ...
            const allContent = customerMessages
              .map((m) => m.content)
              .join(" ")
              .toLowerCase();
      ...
      const scoredArticles = activeArticles.map((article) => {
        ...
        if (article.lastUsedAt) {
          const daysSinceUse =
            (Date.now() - new Date(article.lastUsedAt).getTime()) /
            (1000 * 60 * 60 * 24);
  ...
  app.get("/api/public/knowledge-base/:id/file", async (req, res) => {
    try {
      ...
      if (article.sourceType !== "file" || !article.filePath) {
        return res
          .status(404)
          .json({ error: "No file associated with this article" });
      }
      ...
      res.setHeader(
        "Content-Disposition",
        `inline; filename="${article.fileName || "document"}"`,
      );
  ...
  app.post("/api/public/knowledge-base/:id/feedback", async (req, res) => {
    try {
      ...
      if (typeof helpful !== "boolean") {
        return res
          .status(400)
          .json({ error: "Helpful field must be a boolean" });
      }
  ...
  app.post("/api/public/support/search", async (req, res) => {
    try {
      ...
      if (!publicAgent) {
        return res
          .status(503)
          .json({ error: "Support service temporarily unavailable" });
      }
      ...
      const knowledgeContent = searchResults.map(
        (r) => `[${r.title}]\n${r.content}`,
      );
      ...
      res.json({
        ...
        sources: sources.map((kb, idx) => ({
          ...
          category: kb.category,
          relevanceScore:
            searchResults.find((r) => r.id === kb.id)?.score || 0.8,
        })),
      });
    } catch (error) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch knowledge base article" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const validationResult = insertKnowledgeBaseSchema
          .extend({
        ...
        setImmediate(async () => {
          try {
            ...
            console.log(
              `✅ Successfully indexed manually created article ${newArticle.id} for AI search`,
            );
          } catch (indexError) {
            // Mark as failed with error message
            await storage.updateKnowledgeBase(newArticle.id, {
            ...
            console.error(
              `⚠️ Warning: Failed to index article ${newArticle.id}:`,
              indexError,
            );
          }
        });
        ...
        if (validationResult.data.assignedAgentIds) {
          await syncAgentKnowledgeAssignments(
            newArticle.id,
            validationResult.data.assignedAgentIds,
          );
        }
        ...
        res
          .status(500)
          .json({ error: "Failed to create knowledge base article" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        // Validate using Drizzle-Zod schema
        const validationResult = updateKnowledgeBaseSchema
          .extend({
        ...
        setImmediate(async () => {
          try {
            ...
            console.log(
              `✅ Successfully re-indexed updated article ${id} for AI search`,
            );
            ...
            console.error(
              `⚠️ Warning: Failed to re-index article ${id}:`,
              indexError,
            );
          }
        });
        ...
        res
          .status(500)
          .json({ error: "Failed to update knowledge base article" });
      }
    },
  );
      ...
      try {
          ...
          try {
            if (fs.existsSync(image.filePath)) {
              ...
              if (!resolvedFilePath.startsWith(resolvedUploadDir)) {
                console.error(
                  `Security: Attempted to delete file outside upload directory: ${image.filePath}`,
                );
                return;
              }
            ...
            console.error(
              `Failed to delete image file ${image.filePath}:`,
              fileError,
            );
        ...
        const imageDeletePromises = associatedImages.map(async (image) => {
          try {
            ...
            console.error(
              `Failed to delete image record ${image.id}:`,
              dbError,
            );
        ...
        deleteArticleFromShre(id).catch((err) =>
          console.warn(
            `[ShreGateway] KB delete sync failed: ${err instanceof Error ? err.message : err}`,
          ),
        );
        ...
        res
          .status(500)
          .json({ error: "Failed to delete knowledge base article" });
      }
    },
  );
  ...
  app.post(
    "/api/knowledge-base/reindex-all",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        const pendingArticles = articles.filter(
          (a) => a.isActive && a.indexingStatus !== "indexed",
        );
        ...
        (async () => {
          const knowledgeRetrievalService =
            KnowledgeRetrievalService.getInstance();
          let successCount = 0;
          let failCount = 0;
          ...
          console.log(
            `Bulk reindexing complete: ${successCount} succeeded, ${failCount} failed`,
          );
        })();
  ...
  app.post(
    "/api/knowledge-base/sync-shared",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        const orgArticles = allArticles.filter(
          (a) => a.organizationId === orgId && a.isActive,
        );
        ...
        console.log(
          `sync-shared: synced ${synced}/${orgArticles.length} articles into collection ${collectionId}`,
        );
        res.json({
          success: true,
          collectionId,
    ...
    async (req, res) => {
      if (!isShreEnabled) {
        return res
          ...
          .json({
            error:
              "Shre AI not configured. Set SHRE_API_KEY environment variable.",
          });
      }
      ...
      try {
        ...
        console.log(
          `[Shre Sync] Starting bulk sync of ${activeArticles.length} articles to Shre AI...`,
        );
        ...
        console.log(
          `[Shre Sync] Complete: ${result.synced} synced, ${result.errors} errors`,
        );
        res.json({
          success: true,
    ...
    async (req, res) => {
      try {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          const uploadedImage =
            await storage.createKnowledgeBaseImage(imageData);
        ...
        res
          .status(500)
          .json({ error: "Failed to upload knowledge base images" });
      }
    },
  );
  ...
  app.delete(
    "/api/knowledge-base/:articleId/images/:imageId",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        if (fs.existsSync(imageToDelete.filePath)) {
          ...
          if (!resolvedFilePath.startsWith(resolvedUploadDir)) {
            console.error(
              `Security: Attempted to delete file outside upload directory: ${imageToDelete.filePath}`,
            );
            return res.status(403).json({ error: "Access denied" });
          }
        ...
        res
          .status(500)
          .json({ error: "Failed to delete knowledge base image" });
      }
    },
  );
  ...
  // Update image order
  app.put(
    "/api/knowledge-base/:articleId/images/:imageId/order",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        if (typeof displayOrder !== "number") {
          return res
            .status(400)
            .json({ error: "Display order must be a number" });
        }
  ...
  app.get(
    "/api/knowledge-base/images/:filename",
    requireAuth,
    requireRole(["admin", "agent"]),
    (req, res) => {
      try {
  ...
  app.post(
    "/api/knowledge-base/:id/videos/youtube",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        if (!title || !youtubeUrl) {
          return res
            .status(400)
            .json({ error: "Title and YouTube URL are required" });
        }
        ...
        if (!youtubeId) {
          return res
            .status(400)
            .json({ error: "Could not extract video ID from YouTube URL" });
        }
  ...
  app.post(
    "/api/knowledge-base/:id/videos/upload",
    requireAuth,
    requireRole(["admin", "agent"]),
  ...
  app.delete(
    "/api/knowledge-base/:articleId/videos/:videoId",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to delete knowledge base video" });
      }
    },
  );
  ...
  // Update video order
  app.put(
    "/api/knowledge-base/:articleId/videos/:videoId/order",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
      ...
      try {
        ...
        if (!shouldUseAI && !category) {
          return res
            .status(400)
            .json({
              error: "Category is required when AI analysis is disabled",
            });
        }
        ...
        for (const file of files) {
          try {
            ...
            // Format content for knowledge base
            const content =
              DocumentProcessor.formatForKnowledgeBase(documentContent);

            console.log(
              `Successfully extracted text from ${file.originalname}: ${documentContent.metadata?.wordCount || 0} words`,
            );
            ...
            if (assignedAgentIds) {
              try {
                ...
                console.warn(
                  "Failed to parse assignedAgentIds, using empty array",
                );
                finalAgentIds = [];
              }
            }
            ...
            if (shouldUseAI) {
              try {
                console.log(
                  `[AI Analysis] Analyzing document: ${file.originalname}`,
                );
                aiAnalysis = await AIDocumentAnalyzer.analyzeDocument(
                  content,
                  finalTitle,
                );
                ...
                if (!category && aiAnalysis.category) {
                  ...
                  console.log(
                    `[AI Analysis] Using AI-suggested category: ${finalCategory}`,
                  );
                }
                ...
                if (
                  ...
                  console.log(
                    `[AI Analysis] Using AI-suggested tags: ${finalTags.join(", ")}`,
                  );
                } else if (tags && aiAnalysis.tags.length > 0) {
                  // Merge manual tags with AI tags, removing duplicates
                  finalTags = Array.from(
                    new Set([...finalTags, ...aiAnalysis.tags]),
                  );
                  console.log(
                    `[AI Analysis] Merged tags: ${finalTags.join(", ")}`,
                  );
                }
                ...
                if (aiAnalysis.suggestedTitle) {
                  ...
                  console.log(
                    `[AI Analysis] Using AI-suggested title: ${finalTitle}`,
                  );
                }

                // Auto-assign to AI agents based on category if not manually assigned
                if (
                  (!assignedAgentIds || finalAgentIds.length === 0) &&
                  aiAnalysis
                ) {
                  const agentSuggestion =
                    AIDocumentAnalyzer.suggestAgentAssignment(aiAnalysis);
                  const allAgents = await storage.getAllAiAgents();
                  ...
                  console.log(
                    `[AI Analysis] Auto-assigned to agents: ${agentSuggestion.suggestedAgentNames.join(", ")}`,
                  );
                }
              } catch (aiError) {
                console.error(
                  `[AI Analysis] Error analyzing document, proceeding with manual inputs:`,
                  aiError,
                );
              }
            }
            ...
            setImmediate(async () => {
              try {
                ...
                console.log(
                  `✅ Successfully indexed uploaded document ${article.id} for AI search`,
                );
              } catch (indexError) {
                // Mark as failed with error message
                await storage.updateKnowledgeBase(article.id, {
                ...
                console.error(
                  `⚠️ Warning: Failed to index article ${article.id}:`,
                  indexError,
                );
              }
            });
            ...
            setImmediate(async () => {
              try {
                console.log(
                  `[Image Extraction] Starting extraction for article ${article.id} from ${file.originalname}`,
                );
                ...
                if (imageResult.images.length > 0) {
                  ...
                  console.log(
                    `✅ Extracted ${imageResult.images.length} images from ${file.originalname} (${(imageResult.totalSize / 1024).toFixed(1)}KB total)`,
                  );
                } else {
                ...
                console.error(
                  `⚠️ Warning: Failed to extract images from ${file.originalname}:`,
                  imageError,
                );
              }
            });
            ...
            if (aiAnalysis && aiAnalysis.faqs && aiAnalysis.faqs.length > 0) {
              console.log(
                `[AI Analysis] Creating ${aiAnalysis.faqs.length} FAQs for article: ${article.title}`,
              );
            ...
            if (finalAgentIds.length > 0) {
              for (const agentId of finalAgentIds) {
                try {
                  ...
                  console.error(
                    `Error updating agent ${agentId} with knowledge base:`,
                    agentError,
                  );
                }
              }
        ...
        console.error(
          "Failed to create knowledge base articles from files:",
          error,
        );
        res
          .status(500)
          .json({
            error: "Failed to create knowledge base articles from files",
          });
      }
    },
  );
    ...
    async (req, res) => {
      try {
        ...
        const formattedContent =
          WebScraper.formatForKnowledgeBase(scrapedContent);
        ...
        setImmediate(async () => {
          try {
            ...
            console.log(
              `✅ Successfully indexed URL-imported article ${article.id} for AI search`,
            );
          } catch (indexError) {
            // Mark as failed with error message
            await storage.updateKnowledgeBase(article.id, {
        ...
        console.log(
          `Successfully created knowledge base article from URL: ${url}, Article ID: ${article.id}`,
        );
        ...
        console.error(
          "Failed to create knowledge base article from URL:",
          error,
        );
        ...
        if (error instanceof Error) {
          if (error.message.includes("timeout")) {
            return res
              ...
              .json({
                error:
                  "URL request timeout - the website took too long to respond",
              });
          }
          if (
            ...
            return res
              .status(413)
              .json({
                error: "Content too large - the webpage exceeds the size limit",
              });
          }
          if (error.message.includes("content type")) {
            return res
              ...
              .json({
                error:
                  "Unsupported content type - only HTML pages are supported",
              });
          }
          if (error.message.includes("No meaningful content")) {
            return res
              .status(400)
              .json({ error: "No meaningful content found on the page" });
          }
          if (
          ...
          if (
            ...
            return res
              .status(400)
              .json({ error: `Website error: ${error.message}` });
          }
          if (
            ...
            return res
              .status(400)
              .json({
                error: "Unable to resolve hostname - please check the URL",
              });
          }
        }
      ...
      try {
        ...
        for (const file of files) {
          try {
            ...
            const existingFile =
              await storage.getUploadedFileByHash(sha256Hash);
            ...
            if (existingFile) {
              ...
              const uploadedFile =
                await storage.createUploadedFile(duplicateData);
              results.push({
                file: uploadedFile,
                isDuplicate: true,
              ...
              setTimeout(async () => {
                try {
                  await processFileForAITraining(
                    uploadedFile,
                    category,
                    parsedTags,
                    user.id,
                  ...
                  console.error(
                    `Failed to process file ${uploadedFile.originalName} for AI training:`,
                    error,
                  );
                  const errorMessage =
  ...
  app.post(
    "/api/files/:fileId/link-knowledge-base/:knowledgeBaseId",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        const link = await storage.linkFileToKnowledgeBase(
          fileId,
          knowledgeBaseId,
        );
        ...
        res
          .status(500)
          .json({ error: "Failed to link file to knowledge base" });
      }
    },
  );
  ...
  app.delete(
    "/api/files/:fileId/unlink-knowledge-base/:knowledgeBaseId",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to unlink file from knowledge base" });
      }
    },
  );
  ...
  app.get(
    "/api/knowledge-base/:id/linked-files",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
  ...
  app.get(
    "/api/analytics/knowledge-articles",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch top knowledge articles" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch agent workload metrics" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const files = await storage.getTopUsedFiles(
          parsedLimit,
          agentId as string,
        );
        res.json(files);
  ...
  app.get(
    "/api/analytics/files/effectiveness",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        const effectiveness =
          await storage.getFileEffectivenessMetrics(parsedLimit);
        res.json(effectiveness);
      } catch (error) {
        console.error("Failed to fetch file effectiveness metrics:", error);
        res
          .status(500)
          .json({ error: "Failed to fetch file effectiveness metrics" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch agent file usage analytics" });
      }
    },
  );
  ...
  app.post("/api/feed/posts/:id/comments", requireAuth, async (req, res) => {
    try {
      ...
      const commentData = insertPostCommentSchema.parse({
        ...
        authorId: user.id,
        authorType:
          user.role === "admin" || user.role === "agent" ? "staff" : "customer",
  ...
  app.post("/api/feed/posts/:id/view", requireAuth, async (req, res) => {
    try {
      ...
      const view = await storage.recordPostView(
        req.params.id,
        userId,
        userType,
      );
  ...
  app.get(
    "/api/feed/posts/:filter(all|internal|urgent|targeted|customer)?",
    async (req, res) => {
      try {
        ...
        if (filter === "internal" && userType !== "staff") {
          return res
            .status(403)
            .json({ error: "Access denied to internal posts" });
        }
  ...
  app.get(
    "/api/widget/conversations/:customerId",
    validateApiKey,
    ...
    async (req, res) => {
      try {
        ...
        const conversationMessages =
          await storage.getCustomerChatMessages(conversationId);
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const { name, description, slug, organizationId, isDefault, settings } =
          req.body;
        const workspace = await storage.createWorkspace({
          name,
          description,
      ...
      try {
        ...
        const updatedSettings = {
          ...
          features: {
            ...
            voiceChat:
              voiceChat ?? currentSettings.features?.voiceChat ?? false,
          },
        };
  ...
  app.get("/api/workspaces/:id/members", requireAuth, async (req, res) => {
    try {
      const members = await storage.getWorkspaceMembersByWorkspace(
        req.params.id,
      );
      res.json(members);
    } catch (error) {
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        const member = await storage.updateWorkspaceMember(
          req.params.id,
          req.body,
        );
        res.json(member);
  ...
  app.get("/api/departments", requireAuth, async (req, res) => {
    try {
      ...
      if (workspaceId) {
        const departments =
          await storage.getDepartmentsByWorkspace(workspaceId);
  ...
  app.get("/api/workspaces/:id/departments", requireAuth, async (req, res) => {
    try {
      const departments = await storage.getDepartmentsByWorkspace(
        req.params.id,
      );
      res.json(departments);
    } catch (error) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const user = req.user as any;
        const { name, description, slug, workspaceId, isDefault, icon, color } =
          req.body;
  ...
  app.get("/api/departments/:id/members", requireAuth, async (req, res) => {
    try {
      const members = await storage.getDepartmentMembersByDepartment(
        req.params.id,
      );
      res.json(members);
    } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        if (!workspaceMemberId) {
          return res
            .status(400)
            .json({ error: "workspaceMemberId is required" });
        }
  ...
  app.put(
    "/api/department-members/:id",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        const member = await storage.updateDepartmentMember(
          req.params.id,
          req.body,
        );
        res.json(member);
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        const account = await channelService.updateChannelAccount(
          req.params.id,
          req.body,
        );
        if (!account) {
  ...
  app.post(
    "/api/channel-accounts/:id/test",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        const result = await channelService.testChannelConnection(
          req.params.id,
        );
        res.json(result);
      } catch (error) {
  ...
  // Template Management
  app.get(
    "/api/channel-accounts/:id/templates",
    requireAuth,
    async (req, res) => {
      try {
        const templates = await channelService.getTemplatesByAccount(
          req.params.id,
        );
        res.json(templates);
      } catch (error) {
  ...
  // Bot Control Routes
  app.post(
    "/api/conversations/:id/pause-bot",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/conversations/:id/resume-bot",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.get(
    "/api/conversations/:id/session-window",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.get("/api/leads/stats", requireAuth, async (req, res) => {
    try {
      ...
      const stats = {
        ...
        avgScore:
          leads.length > 0
            ? leads.reduce((sum, l) => sum + (l.leadScore || 0), 0) /
              leads.length
            : 0,
      };
  ...
  app.put("/api/leads/:id", requireAuth, async (req, res) => {
    try {
      const { leadStatus, leadScore, businessName, businessType, notes, tags } =
        req.body;

      const [updated] = await db
        .update(channelContacts)
        .set({
          leadStatus,
          leadScore,
          businessName,
  ...
  app.get(
    "/api/conversations/:id/channel-meta",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.post("/api/webhooks/channel/:accountId", async (req, res) => {
    ...
    try {
      ...
      processWebhook(account, req.body, req.headers, startTime).catch(
        (error) => {
  ...
  async function processWebhook(
    ...
    try {
      const provider = channelProviderFactory.getProvider(
        account.provider as "meta_cloud" | "twilio",
      );
      ...
      if (inboundMessage) {
        ...
        if (!customer) {
          ...
          customer = await storage.createCustomer({
            ...
            email:
              contact.email ||
              `${inboundMessage.senderId}@${account.channelType}.channel`,
        ...
        let conversation = await storage.findOpenConversationByCustomer(
          customer.id,
        );
  ...
  const { platformAssistantService } = await import(
  ...
  app.get(
    "/api/platform-assistant/conversations",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as any;
        const conversations = await platformAssistantService.getConversations(
          user.id,
        );
  ...
  app.get(
    "/api/platform-assistant/conversations/:id/messages",
    requireAuth,
    async (req, res) => {
      try {
        const messages = await platformAssistantService.getConversationMessages(
          req.params.id,
        );
        res.json(messages);
      } catch (error) {
  ...
  app.get(
    "/api/platform-assistant/suggestions",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as any;
        const suggestions = platformAssistantService.getSuggestedQuestions(
          user.role,
        );
  ...
  // Get quick actions
  app.get(
    "/api/platform-assistant/quick-actions",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as any;
  ...
  app.post(
    "/api/platform-assistant/execute-action",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        const result = await platformAssistantService.executeAction(
          actionId,
          parameters || {},
          context,
        );
  ...
  app.get(
    "/api/knowledge-scheduler/status",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/knowledge-scheduler/process-stale",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/knowledge-scheduler/reindex-all",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/webhooks/knowledge-base/update",
    webhookRateLimiter,
    async (req, res) => {
      try {
        ...
        if (action === "reindex-all") {
          ...
          action === "reindex-articles" &&
          Array.isArray(articleIds) &&
          articleIds.length <= 50
        ) {
          const knowledgeRetrieval = KnowledgeRetrievalService.getInstance();
          let processed = 0;
          let errors = 0;
          ...
          for (const articleId of articleIds) {
            try {
              ...
              errors++;
              console.error(
                `Webhook: Failed to reindex article ${articleId}:`,
                err,
              );
            }
          }
          ...
          return res
            .status(400)
            .json({ error: "articleIds must be an array with max 50 items" });
        } else {
          return res
            .status(400)
            .json({
              error:
                "Invalid action. Use: reindex-all, reindex-stale, or reindex-articles",
            });
        }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!webhookSecret) {
          console.warn(
            "[Shre Provision] SHRE_WEBHOOK_SECRET not configured — webhook disabled",
          );
          return res
            .status(503)
            .json({ error: "Provisioning webhook not configured" });
        }
        ...
        if (!orgSlug || !companyId) {
          return res
            .status(400)
            .json({ error: "orgSlug and companyId are required" });
        }

        console.log(
          `[Shre Provision] Received provisioning for org=${orgSlug} company=${companyId}`,
        );
          ...
          setImmediate(async () => {
            try {
              ...
              if (activeArticles.length > 0) {
                ...
                const result = await bulkSyncKBToShre(mapped);
                console.log(
                  `[Shre Provision] Auto-synced ${result.synced} KB articles to Shre`,
                );
              }
            } catch (err) {
  ...
  app.get("/api/onboarding/progress", requireAuth, async (req, res) => {
    try {
      const user = req.user as any;
      const progress = await platformAssistantService.getOnboardingProgress(
        user.id,
      );
      res.json(progress);
    } catch (error) {
  ...
  app.post("/api/onboarding/complete", requireAuth, async (req, res) => {
    try {
      ...
      await platformAssistantService.markOnboardingComplete(
        user.id,
        checklistItemId,
        metadata,
      );
  ...
  app.get("/api/onboarding/checklist", requireAuth, async (req, res) => {
    try {
      const { ONBOARDING_CHECKLIST } = await import(
  ...
  const { documentationGeneratorService } = await import(
      ...
      try {
        ...
        if (!integrationName || !integrationType) {
          return res
            .status(400)
            .json({
              error: "integrationName and integrationType are required",
            });
        }

        const result =
          await documentationGeneratorService.createAndSaveDocumentation(
            {
              integrationName,
              integrationType,
              providerName,
  ...
  app.post(
    "/api/documentation/generate-channel-guides",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        const user = req.user as any;
        const results =
          await documentationGeneratorService.generateChannelSetupGuides(
            user.id,
          );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const preview =
          await documentationGeneratorService.generateIntegrationDocumentation({
            integrationName,
            integrationType,
            providerName,
            features,
  ...
  app.get("/api/docs/documents", requireAuth, async (req, res) => {
    try {
      ...
      const documents = await storage.getDocumentsByWorkspace(
        workspaceId,
        filters,
      );
      res.json(documents);
  ...
  app.get("/api/docs/documents/:id/versions", requireAuth, async (req, res) => {
    try {
      const versions = await storage.getDocumentVersionsByDocument(
        req.params.id,
      );
      res.json(versions);
    } catch (error) {
  ...
  app.put(
    "/api/docs/versions/:id",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        const version = await storage.updateDocumentVersion(
          req.params.id,
          req.body,
        );
        res.json(version);
  ...
  app.get(
    "/api/docs/documents/:id/relationships",
    requireAuth,
    async (req, res) => {
      try {
        const relationships = await storage.getDocumentRelationships(
          req.params.id,
        );
  ...
  app.post(
    "/api/docs/review-queue/:id/approve",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        const user = req.user as any;
        await storage.approveDocumentReview(
          req.params.id,
          user.id,
          req.body.notes,
        );
  ...
  app.post(
    "/api/docs/review-queue/:id/reject",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        await storage.rejectDocumentReview(
          req.params.id,
          user.id,
          req.body.notes,
        );
  ...
  app.get("/api/docs/ai-export", requireAuth, async (req, res) => {
    try {
      ...
      const documents = await storage.getDocumentsForAIExport(
        workspaceId,
        filters,
      );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const storedName =
          file.path.split("/").pop() || file.filename || `upload-${Date.now()}`;
    ...
    try {
      ...
      if (conversationId) {
        try {
          ...
          const kbLinks =
            aiResponse.knowledgeLinks.length > 0
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const usage = await storage.getDailyTokenUsage(
          workspaceId || null,
          days,
        );
        res.json(usage);
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const usage = await storage.getMonthlyTokenUsage(
          workspaceId || null,
          months,
        );
        res.json(usage);
  ...
  app.get(
    "/api/admin/ai-usage/conversation/:conversationId",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        const { conversationId } = req.params;
        const usage =
          await storage.getAiTokenUsageByConversation(conversationId);
        ...
        res.json({
          usage,
          totals: { ...totals, totalCost: totals.totalCost.toFixed(6) },
        });
      } catch (error) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const monthlyData = await storage.getAiTokenUsageSummary(
          workspaceId || null,
          startOfMonth,
        );

        // Calculate current month totals
        const currentMonthTotals = monthlyData.reduce(
          (acc, u) => ({
            totalTokens: acc.totalTokens + u.totalTokens,
        ...
        const dailyData = await storage.getDailyTokenUsage(
          workspaceId || null,
          30,
        );
  ...
  app.get(
    "/api/admin/ai-learning/top-articles",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.get(
    "/api/admin/ai-learning/needs-improvement",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/admin/ai-learning/update-outcome",
    requireAuth,
    requireRole(["admin", "agent"]),
    async (req, res) => {
      try {
        ...
        if (!conversationId || !outcome) {
          return res
            .status(400)
            .json({ error: "conversationId and outcome are required" });
        }

        await storage.updateKnowledgeFeedbackByConversation(
          conversationId,
          outcome,
          customerRating,
        );
  ...
  // Get article metrics
  app.get(
    "/api/admin/ai-learning/article-metrics/:knowledgeBaseId",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        const metrics =
          await storage.getKnowledgeArticleMetrics(knowledgeBaseId);
    ...
    async (req, res) => {
      try {
        ...
        if (existing) {
          return res
            .status(409)
            .json({ error: "Region with this ISO code already exists" });
        }
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const updated = await storage.updateRegion(
          req.params.id,
          validation.data,
        );
        res.json(updated);
  ...
  async function verifyOrgAdminAccess(
    userId: string,
    organizationId: string,
  ): Promise<boolean> {
    const membership = await storage.getOrganizationMemberByUser(
      organizationId,
      userId,
    );
    return (
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const userMembership = await storage.getOrganizationMemberByUser(
          req.params.orgId,
          currentUserId,
        );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const currentUserId = (req.user as any)?.id;
        const hasAccess = await verifyOrgAdminAccess(
          currentUserId,
          req.params.orgId,
        );
        if (!hasAccess) {
        ...
        const data = validation.data;
        const existing = await storage.getOrganizationMemberByUser(
          req.params.orgId,
          data.userId,
        );
        if (existing) {
  ...
  app.put("/api/organization-members/:id", requireAuth, async (req, res) => {
    try {
      ...
      const updated = await storage.updateOrganizationMember(
        req.params.id,
        validation.data,
      );
      res.json(updated);
  ...
  app.get("/api/users/:userId/organizations", requireAuth, async (req, res) => {
    try {
      ...
      if (currentUserId !== req.params.userId && currentUserRole !== "admin") {
        return res
          .status(403)
          .json({ error: "You can only view your own organizations" });
      }

      const organizations = await storage.getUserOrganizations(
        req.params.userId,
      );
  ...
  app.get("/api/knowledge-collections", requireAuth, async (req, res) => {
    try {
      ...
      const collections =
        await storage.getAllKnowledgeCollections(organizationId);
  ...
  app.post("/api/knowledge-collections", requireAuth, async (req, res) => {
    try {
      ...
      const existing = await storage.getKnowledgeCollectionBySlug(
        data.slug,
        data.ownerOrganizationId,
      );
      if (existing) {
        return res
          .status(409)
          .json({ error: "Collection with this slug already exists" });
      }
  ...
  app.put("/api/knowledge-collections/:id", requireAuth, async (req, res) => {
    try {
      ...
      const currentUserId = (req.user as any)?.id;
      const hasAccess = await verifyCollectionAccess(
        currentUserId,
        collection,
        "manage",
      );
      ...
      const updated = await storage.updateKnowledgeCollection(
        req.params.id,
        validation.data,
      );
      res.json(updated);
  ...
  app.get(
    "/api/knowledge-collections/:id/articles",
    requireAuth,
    async (req, res) => {
      try {
        const collection = await storage.getKnowledgeCollection(req.params.id);
  ...
  app.delete(
    "/api/knowledge-collections/:collectionId/articles/:articleId",
    requireAuth,
    async (req, res) => {
      try {
        const collection = await storage.getKnowledgeCollection(
        ...
        await storage.removeArticleFromCollection(
          req.params.collectionId,
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const collections = await storage.getWorkspaceCollections(
    ...
    async (req, res) => {
      try {
        const accessLevelSchema = z.object({
          accessLevel: z.enum(["read", "contribute", "manage"]),
        });
        ...
        await storage.updateWorkspaceCollectionAccess(
          req.params.workspaceId,
          req.params.collectionId,
          validation.data.accessLevel,
        );
        res.json({ success: true });
      } catch (error) {
  ...
  async function verifyResolutionAccess(
    userId: string,
    record: { organizationId?: string | null; workspaceId?: string | null },
  ): Promise<boolean> {
    ...
    async (req, res) => {
      try {
        ...
        if (
          ...
          validation.data.organizationId !== userOrgId
        ) {
          return res
            .status(403)
            .json({
              error:
                "Access denied: Cannot create records for other organizations",
            });
        }
        ...
        const record = await storage.createResolutionRecord({
          ...validation.data,
          organizationId:
            validation.data.organizationId || userOrgId || undefined,
  ...
  app.get(
    "/api/customers/:customerId/resolutions",
    requireAuth,
    async (req, res) => {
      try {
        const currentUserId = (req.user as any)?.id;
        ...
        const limit = parseInt(req.query.limit as string) || 10;
        const resolutions = await storage.getResolutionsByCustomer(
          req.params.customerId,
          limit,
        );

        // Filter to only resolutions in user's organization
        const scopedResolutions = resolutions.filter(
          (r) => !r.organizationId || r.organizationId === userOrgId,
        );
  ...
  app.get(
    "/api/customers/:customerId/resolution-summary",
    requireAuth,
    async (req, res) => {
      try {
        const currentUserId = (req.user as any)?.id;
        ...
        const summary = await storage.getResolutionSummaryForCustomer(
          req.params.customerId,
        );
        res.json(summary);
      } catch (error) {
  ...
  app.get(
    "/api/customers/:customerId/resolutions/category/:category",
    requireAuth,
    async (req, res) => {
      try {
        const currentUserId = (req.user as any)?.id;
  ...
  app.get(
    "/api/customers/:customerId/successful-resolutions",
    requireAuth,
    async (req, res) => {
      try {
        const currentUserId = (req.user as any)?.id;
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const updated = await storage.updateResolutionRecord(
          req.params.id,
          validation.data,
        );
        res.json(updated);
  ...
  app.get(
    "/api/workspaces/:workspaceId/resolutions",
    requireAuth,
    async (req, res) => {
      try {
        const currentUserId = (req.user as any)?.id;
        ...
        const resolutions = await storage.getResolutionsByWorkspace(
          req.params.workspaceId,
          options,
        );
  ...
  app.get("/api/workflows", requireAuth, async (req, res) => {
    try {
      ...
      const playbooks = await storage.getWorkflowPlaybooks(workspaceId, {
        status,
        category,
      });
      res.json(playbooks);
    ...
    async (req, res) => {
      try {
        const schema = z.object({
          ...
          visibility: z
            .enum(["workspace", "organization", "global"])
            .default("workspace"),
        });
        ...
        if (workspaceId === "default") {
          ...
          if (!defaultWorkspace) {
            return res
              .status(400)
              .json({ error: "No default workspace found" });
          }
          workspaceId = defaultWorkspace.id;
        }
  ...
  app.get("/api/workflows/:playbookId/nodes", requireAuth, async (req, res) => {
    try {
      const nodes = await storage.getWorkflowNodesByPlaybook(
  ...
  app.post(
    "/api/workflows/:playbookId/nodes",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        const schema = z.object({
          ...
          resolutionType: z
            .enum(["success", "escalate", "external"])
            .optional(),
        ...
        if (validation.data.isEntryPoint) {
          await storage.updateWorkflowPlaybook(req.params.playbookId, {
            startNodeId: node.id,
          });
        }
  ...
  app.put(
    "/api/workflows/:playbookId/nodes/:nodeId",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        const node = await storage.updateWorkflowNode(
          req.params.nodeId,
          validation.data,
        );
        res.json(node);
  ...
  app.get("/api/workflows/:playbookId/edges", requireAuth, async (req, res) => {
    try {
      const edges = await storage.getWorkflowEdgesByPlaybook(
        req.params.playbookId,
      );
      res.json(edges);
    } catch (error) {
  ...
  app.post(
    "/api/workflows/:playbookId/edges",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        const schema = z.object({
          ...
          conditionType: z
            .enum(["default", "option", "expression"])
            .default("default"),
  ...
  app.delete(
    "/api/workflows/:playbookId/edges/:edgeId",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/conversations/:conversationId/workflow-session",
    requireAuth,
    async (req, res) => {
      try {
        const schema = z.object({
        ...
        const workflow = await storage.getFullWorkflow(
          validation.data.playbookId,
        );
        if (!workflow) {
          return res.status(404).json({ error: "Workflow not found" });
        }
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const session = await storage.getWorkflowSessionByConversation(
  ...
  app.post(
    "/api/workflow-sessions/:sessionId/progress",
    requireAuth,
    async (req, res) => {
      try {
        const schema = z.object({
        ...
        const edges = await storage.getWorkflowEdgesBySource(
        ...
        if (validation.data.answer) {
          const matchingEdge = edges.find(
            (e) =>
              e.conditionType === "option" &&
              e.conditionValue === validation.data.answer,
          );
        ...
        if (!nextEdge) {
          return res
            .status(400)
            .json({ error: "No valid path from current node" });
        }
        ...
        const updated = await storage.updateWorkflowSession(
  ...
  app.get("/api/workflows/search", requireAuth, async (req, res) => {
    try {
      ...
      const keywords = query
        ...
        .split(/\s+/)
        .filter((w) => w.length > 2);
      const matches = await storage.searchWorkflowsByKeywords(
        workspaceId,
        keywords,
      );
  ...
  app.get(
    "/api/cloud-storage/oauth-configs",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
      ...
      try {
        ...
        const existing = await db
          ...
          .where(
            and(
              eq(
                cloudStorageOAuthConfigs.workspaceId,
        ...
        if (existing.length > 0) {
          ...
          const [updated] = await db
            .update(cloudStorageOAuthConfigs)
            .set(updateData)
        ...
        if (!validatedData.clientSecret) {
          return res
            .status(400)
            .json({ error: "Client Secret required for new configurations" });
        }

        // Create new config
        const [newConfig] = await db
          .insert(cloudStorageOAuthConfigs)
          .values({
            organizationId: user.organizationId,
  ...
  // Delete OAuth config
  app.delete(
    "/api/cloud-storage/oauth-configs/:id",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        await db
          .delete(cloudStorageOAuthConfigs)
  ...
  const validateConnectionOwnership = async (
    ...
    if (!isAdminUser(user) && connection.workspaceId !== user.workspaceId)
      return null;
    return connection;
  };
    ...
    try {
      ...
      if (isAdminUser(user)) {
        const allConnections =
          await storage.getCloudStorageConnectionsByOrganization(
            user.organizationId,
          );
      ...
      if (requestedWorkspaceId && requestedWorkspaceId !== user.workspaceId) {
        return res
          .status(403)
          .json({ error: "Access denied: cannot query other workspaces" });
      }
      ...
      const connections =
        await storage.getCloudStorageConnectionsByWorkspace(workspaceId);
  ...
  app.get(
    "/api/cloud-storage/connections/:id",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as User;
        const connection = await validateConnectionOwnership(
          req.params.id,
          user,
        );
        if (!connection) {
  ...
  app.get(
    "/api/cloud-storage/connections/:id/folders",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as User;
        ...
        const folders = await storage.getCloudStorageFoldersByConnection(
          req.params.id,
        );
        res.json(folders);
      } catch (error) {
  ...
  app.get(
    "/api/cloud-storage/connections/:id/sync-runs",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as User;
        ...
        const limit = parseInt(req.query.limit as string) || 10;
        const runs = await storage.getCloudStorageSyncRunsByConnection(
          req.params.id,
          limit,
        );
        res.json(runs);
  ...
  app.post(
    "/api/cloud-storage/connections/:id/sync",
    requireAuth,
    async (req, res) => {
      try {
        const user = req.user as User;
        ...
        runCloudStorageSync(connection.id)
          .then(async (result) => {
            ...
            console.log(
              `[CloudSync] Connection ${connection.id}: ${result.filesImported} imported, ${result.filesSkipped} skipped`,
            );
          })
          .catch(async (err) => {
            console.error("[CloudSync] Sync error:", err);
            await storage
              .updateCloudStorageSyncRun(syncRun.id, {
                status: "failed",
                completedAt: new Date(),
              ...
              .catch(() => {});
            await storage
              .updateCloudStorageConnection(connection.id, {
                status: "error",
                errorMessage: err.message,
              })
              .catch(() => {});
  ...
  app.get(
    "/api/cloud-storage/oauth/:provider/initiate",
    requireAuth,
    async (req, res) => {
      try {
        ...
        const user = req.user as User;
        const workspaceId =
          (req.query.workspaceId as string) || user.workspaceId;
  ...
  app.get("/api/cloud-storage/oauth/:provider/callback", async (req, res) => {
    try {
      ...
      if (error) {
        return res.redirect(
          `/cloud-storage?error=${encodeURIComponent(error as string)}`,
        );
      }
  ...
  const verifyEmailIntegrationOwnership = async (
  ...
  const verifyEmailMessageOwnership = async (
    messageId: string,
    organizationId: string,
  ) => {
    ...
    const integration = await storage.getEmailIntegration(
  ...
  const verifyEmailTemplateOwnership = async (
    templateId: string,
    organizationId: string,
  ) => {
  ...
  const verifyEmailAutoReplyRuleOwnership = async (
    ruleId: string,
    organizationId: string,
  ) => {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const integrations = await storage.getEmailIntegrationsByOrganization(
          user.organizationId,
        );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const integration = await verifyEmailIntegrationOwnership(
          req.params.id,
          user.organizationId,
        );
        if (!integration) {
    ...
    async (req, res) => {
      try {
        ...
        if (!inboundEmail || !provider) {
          return res
            .status(400)
            .json({ error: "inboundEmail and provider are required" });
        }
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const existing = await verifyEmailIntegrationOwnership(
          req.params.id,
          user.organizationId,
        );
        if (!existing) {
        ...
        const integration = await storage.updateEmailIntegration(
          req.params.id,
          req.body,
        );
        res.json(sanitizeEmailIntegration(integration));
  ...
  app.post(
    "/api/email-integrations/:id/test",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
  ...
  app.get("/api/email-messages", requireAuth, async (req, res) => {
    try {
      ...
      const messages = await storage.getEmailMessagesByOrganization(
        user.organizationId,
        {
          limit: limit ? parseInt(limit as string) : undefined,
  ...
  app.get("/api/email-messages/:id", requireAuth, async (req, res) => {
    try {
      ...
      const message = await verifyEmailMessageOwnership(
        req.params.id,
        user.organizationId,
      );
      if (!message) {
      ...
      const attachments = await storage.getEmailAttachmentsByMessage(
        req.params.id,
      );
  ...
  app.patch("/api/email-messages/:id/assign", requireAuth, async (req, res) => {
    try {
      ...
      await storage.updateEmailMessage(req.params.id, {
  ...
  app.get("/api/email-templates", requireAuth, async (req, res) => {
    try {
      ...
      const templates = await storage.getEmailTemplatesByOrganization(
        user.organizationId,
      );
      res.json(templates);
    } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        if (!name || !subject || !bodyHtml) {
          return res
            .status(400)
            .json({ error: "name, subject, and bodyHtml are required" });
        }
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const existing = await verifyEmailTemplateOwnership(
          req.params.id,
          user.organizationId,
        );
        if (!existing) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const rules = await storage.getEmailAutoReplyRulesByOrganization(
          user.organizationId,
        );
        res.json(rules);
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        if (!name || !triggerType) {
          return res
            .status(400)
            .json({ error: "name and triggerType are required" });
        }
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const existing = await verifyEmailAutoReplyRuleOwnership(
          req.params.id,
          user.organizationId,
        );
        if (!existing) {
        ...
        const rule = await storage.updateEmailAutoReplyRule(
          req.params.id,
          req.body,
        );
        res.json(rule);
  ...
  app.get("/api/billing/organization-usage", requireAuth, async (req, res) => {
    try {
      ...
      const stats = await storage.getUsageStatsByOrganization(
        user.organizationId,
        start,
        end,
      );
  ...
  app.get(
    "/api/billing/organization-usage/history",
    requireAuth,
    requireRole(["admin"]),
    async (req, res) => {
      try {
        ...
        let usage = await storage.getAiTokenUsageByOrganization(
          user.organizationId,
          start,
          end,
        );
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch organization usage history" });
      }
    },
  );
  ...
  app.get("/api/billing/daily-trend", requireAuth, async (req, res) => {
    try {
      ...
      const trend = await storage.getDailyTokenUsage(
        workspaceId,
  ...
  app.get("/api/billing/monthly-trend", requireAuth, async (req, res) => {
    try {
      ...
      const workspaceId = user.workspaceId || null;
      const trend = await storage.getMonthlyTokenUsage(
        workspaceId,
  ...
  registerEmbedIframeRoutes(routeContext);
  registerConversationFilesRoutes(routeContext);
