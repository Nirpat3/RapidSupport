...
- **Mobile Message Input Enhancement (Latest)**: Optimized customer chat message input for mobile devices. Shortened placeholder from "Type your message..." to "Type message..." for better visibility on small screens. Implemented responsive padding (pr-3 on mobile, pr-12 on desktop) and reduced gaps between action buttons on mobile (gap-0.5 vs gap-1 on desktop). All buttons maintain proper size="icon" variant for consistent sizing. Input field now fully visible on mobile without text cutoff, tested on iPhone SE viewport (375x667).
- **Mobile Camera & Emoji Picker Fixes (Latest)**: Fixed camera functionality on mobile by switching from MediaDevices API to native file input with `capture="environment"` attribute, which reliably opens the device camera app without requiring HTTPS or permission prompts. Implemented auto-close behavior for emoji picker when any other action button (attach file, camera, voice) is clicked, preventing UI overlap and improving mobile UX. Removed complex camera stream code, video preview dialog, and unused state/refs for cleaner codebase.
...
- **System Activity Messages (Latest)**: When conversations change state (closed, resolved, open, pending, follow-up scheduled/cleared), system messages are automatically created and broadcast to all participants. These messages display as centered, subtle notifications in the chat interface with an info icon, showing who performed the action and when. System messages use senderType 'system' and are visible to both customers and staff for transparency.
- **AI Capabilities**: Multi-agent AI response system (OpenAI GPT-4o-mini) for intent classification (sales, technical, billing, general), smart routing to specialized agents (Sales, Technical, Billing, General Support), and agent handoff based on confidence scores. It includes 4-dimensional quality analysis for AI responses, an AI Learning Dashboard for analytics, and continuous improvement mechanisms.
  - **Enhanced AI Intelligence**: Improved system prompts with explicit formatting guidelines for numbered lists, bullet points, URLs, and paragraph breaks. Response format templates enriched with structured examples showing intro → details → summary pattern. Formatting instructions ensure AI responses are scannable, well-organized, and customer-friendly.
  - **RAG Optimization - Phase 1**: Implemented comprehensive RAG best practices for improved knowledge base usage:
    - **Enhanced Retrieval Logging**: Detailed console logging of retrieved chunks with article names, sections, categories, tags, scores, match types, and content previews. Shows top 5 chunks per query with confidence threshold warnings for debugging "AI not using docs" issues.
    - **Answer Grounding**: AI now cites sources using [Article → Section] format. Knowledge context includes source IDs and citation examples. Prompts enforce mandatory citation of facts from knowledge base.
    - **Confidence Thresholds**: AI abstains from answering when best retrieval score < 0.3, preventing hallucinations. Suggests human agent connection instead of guessing.
    - **Improved Search Parameters**: Increased top-k from 5 to 10 results (RAG guide recommends 8-12), raised minimum score thresholds (0.2-0.3 vs 0.1-0.15), increased semantic similarity threshold from 0.1 to 0.15 for better quality filtering.
    - **Metadata-Aware Filtering**: Priority boosting for high-priority articles (>75), stricter filtering thresholds (0.2-0.25 vs 0.15-0.2), larger result sets for complex queries (10 vs 6 results).
  - **RAG Optimization - Phase 2 (Latest)**: Advanced retrieval improvements for production-grade performance:
    - **Query Rewriting**: Extracts entities (products, features, actions, error codes) from conversation context and builds enhanced search queries. Combines last 3 messages with current query for better context understanding. Example: "How do I fix it?" + context about "PAX bluetooth" → "How do I fix it pax bluetooth connection".
    - **MMR Diversity Filtering**: Implements Maximal Marginal Relevance algorithm to avoid returning 10 near-identical chunks from the same article section. Lambda=0.7 balances relevance vs diversity. Prevents redundant results by checking chunk similarity (same article + adjacent indices = high similarity).
    - **Optimized Chunk Strategy**: Reduced chunk size from 500 to 400 words (closer to RAG guide's 300-500 tokens), reduced minimum from 100 to 80 words for better granularity, and adjusted overlap from 75 to 60 words. Section titles properly stored in chunk metadata for better citation support.
