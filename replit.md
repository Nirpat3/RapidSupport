...
- **Communication Module**: Full internal communication system for both staff and customer portal. 10 DB tables (`comm_posts`, `comm_post_targets`, `comm_post_reads`, `comm_post_reactions`, `comm_post_comments`, `comm_channels`, `comm_channel_members`, `comm_channel_messages`, `comm_direct_threads`, `comm_direct_messages`). Four sections per portal: Announcements (workspace→retailer formal notices with tag badges and acknowledgement), Feed (hierarchical post feed with tabs for upstream/downstream content), Community (Slack-style persistent group channels with internal/customer-facing types), Messages (1-on-1 DMs between users). Staff-side includes create/manage announcements with read analytics (X/Y acknowledged), retailer filter on feed, dual Internal+Customer channel management. Routes at `/api/comm/*` (staff) and `/api/customer-portal/comm/*` (portal).
- **Security Hardening**: Helmet.js middleware adding 9+ HTTP security headers (X-Frame-Options, HSTS, X-Content-Type-Options, Referrer-Policy, etc.). Production error sanitization (generic messages to client, full errors in server logs only). DOMPurify (`isomorphic-dompurify`) sanitizing all `dangerouslySetInnerHTML` rendering. Push notification routes require authentication. Policy generation verifies org ownership. Communication PATCH/DELETE endpoints enforce org isolation. Password hash fragments removed from auth logs.
- **Dual PWA Support**: Two installable PWA experiences from one codebase. Staff app: start URL `/dashboard`, indigo theme (`#6366f1`), shortcuts to Conversations/Dashboard/Knowledge Base. Customer/Retailer portal: dynamic manifest at `GET /api/portal-manifest?org=slug`, emerald theme (`#059669`), start URL `/portal/dashboard`, shortcuts to portal routes. Both use the same PNG icon set (72–512px generated from SVG via ImageMagick) plus 4 iOS splash screens. Portal `CustomerPortalLayout` dynamically swaps the manifest link tag and shows a mobile install banner (`beforeinstallprompt`, 7-day dismiss, hidden in standalone mode).
